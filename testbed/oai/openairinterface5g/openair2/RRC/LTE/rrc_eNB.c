    ...
    *DRB_pdcp_config->discardTimer = LTE_PDCP_Config__discardTimer_infinity;
    ...
    switch (ue_context_pP->ue_context.e_rab[i].param.qos.qci) {
      ...
      case 66: // 100ms, 10^-2, p2, non-mission critical  voice , GBR
        LOG_A(RRC, "DRB generated with dedicated RRC reconfiguration: RLC UM \n");
        // RLC
      ...
      case 70: // 200ms, 10^-6, p5.5, mision critical data
        LOG_A(RRC, "DRB generated with dedicated RRC reconfiguration: RLC AM \n");
        // RLC
  ...
  for (i = 0; i < ue_context_pP->ue_context.nb_of_modify_e_rabs; i++) {
    ...
    *DRB_pdcp_config->discardTimer = LTE_PDCP_Config__discardTimer_infinity;
    ...
    switch (ue_context_pP->ue_context.modify_e_rab[i].param.qos.qci) {
      ...
      case 65: // 75ms, 10^-2, p0.7, mission critical voice, GBR
      case 66: // 100ms, 10^-2, p2, non-mission critical  voice , GBR
        LOG_A(RRC, "DRB generated by modfiying dedicated RRC reconfiguration: RLC UM \n");
        // RLC
        DRB_rlc_config->present = LTE_RLC_Config_PR_um_Bi_Directional;
        DRB_rlc_config->choice.um_Bi_Directional.ul_UM_RLC.sn_FieldLength = LTE_SN_FieldLength_size10;
        DRB_rlc_config->choice.um_Bi_Directional.dl_UM_RLC.sn_FieldLength = LTE_SN_FieldLength_size10;
      ...
      case 69: // 60ms, 10^-6, p0.5, mission critical delay sensitive data, Lowest Priority
      case 70: // 200ms, 10^-6, p5.5, mision critical data
        LOG_A(RRC, "DRB generated by modfiying dedicated RRC reconfiguration: RLC AM \n");
        // RLC
        DRB_rlc_config->present = LTE_RLC_Config_PR_am;
        DRB_rlc_config->choice.am.ul_AM_RLC.t_PollRetransmit = LTE_T_PollRetransmit_ms50;
        DRB_rlc_config->choice.am.ul_AM_RLC.pollPDU = LTE_PollPDU_p16;
    ...
    DRB_ul_SpecificParameters->bucketSizeDuration =
      LTE_LogicalChannelConfig__ul_SpecificParameters__bucketSizeDuration_ms50;
    asn1cSeqAdd(&(DRB_configList2)->list, DRB_config);
    LOG_A(RRC, "EPS ID %ld, DRB ID %ld (index %d), QCI %d, priority %ld, LCID %ld LCGID %ld \n",
          *DRB_config->eps_BearerIdentity,
          DRB_config->drb_Identity, i,
          ue_context_pP->ue_context.modify_e_rab[i].param.qos.qci,
          DRB_ul_SpecificParameters->priority,
...
//-----------------------------------------------------------------------------
{
  ...
  *sr_ProhibitTimer_r9 = 0;   // SR tx on PUCCH, Value in number of SR period(s). Value 0 = no timer for SR, Value 2 = 2*SR
  ...
  if (ue_context_pP->ue_context.does_nr) {
    ...
    MeasObj2->measObject.choice.measObjectNR_r15.carrierFreq_r15 = 641272;//rrc_inst->nr_scg_ssb_freq; //641272; //634000; //(634000 = 3.51GHz) (640000 = 3.6GHz) (641272 = 3619.08MHz = 3600 + 30/1000*106*12/2) (642256 is for 3.6GHz and absoluteFrequencySSB = 642016)
...
//-----------------------------------------------------------------------------
{
  ...
  // PAVE: Add X2-u interface for data forwarding (especially for downlink)
  if (measResults2->measId == 7) {
    if ((ue_context_pP->ue_context.StatusRrc != RRC_NR_NSA) && (ue_context_pP->ue_context.StatusRrc != RRC_NR_NSA_RECONFIGURED)) {
      ...
      // PAVE: How DRB to be added configured?
      if(is_en_dc_supported(ue_context_pP->ue_context.UE_Capability)) {
        ...
        for (int e_rab=0; e_rab < X2AP_ENDC_SGNB_ADDITION_REQ(msg).nb_e_rabs_tobeadded; e_rab++) {
          ...
          // PAVE: Add information for X2U data channel 
          X2AP_ENDC_SGNB_ADDITION_REQ(msg).eNB_gtp_teid[e_rab] = ue_context_pP->ue_context.enb_gtp_teid[e_rab];
          X2AP_ENDC_SGNB_ADDITION_REQ(msg).eNB_addr[e_rab] = ue_context_pP->ue_context.enb_gtp_addrs[e_rab];

          // 240830 PAVE: I guess here is the path swicthing triggering function
          ...
          LOG_A(RRC, "Target addr: %d, %d, %d, %d\n", ue_context_pP->ue_context.enb_gtp_addrs[e_rab].buffer[0],
                                        ue_context_pP->ue_context.enb_gtp_addrs[e_rab].buffer[1],
                                        ue_context_pP->ue_context.enb_gtp_addrs[e_rab].buffer[2],
                                        ue_context_pP->ue_context.enb_gtp_addrs[e_rab].buffer[3]);
        }

        // 240830 PAVE
        LOG_A(RRC, "[eNB %d] frame %d subframe %d: UE rnti %lx switching to NSA mode, msg enb RNTI %lx\n", ctxt_pP->module_id, ctxt_pP->frame, ctxt_pP->subframe, ctxt_pP->rntiMaybeUEid, X2AP_ENDC_SGNB_ADDITION_REQ(msg).rnti);
        itti_send_msg_to_task(TASK_X2AP, ENB_MODULE_ID_TO_INSTANCE(ctxt_pP->module_id), msg);

        // PAVE: Create X2-U interface
        //rrc_eNB_process_X2AP_TUNNEL_SETUP_REQ(ctxt_pP->module_id, ue_context_pP);        
...
//-----------------------------------------------------------------------------
{
  ...
  RB_FOREACH(ue_context_p, rrc_ue_tree_s, &(RC.rrc[ctxt_pP->module_id]->rrc_ue_head)) {
    ...
    if (ue_context_p->ue_context.StatusRrc == RRC_RECONFIGURED
      ...
      // PAVE: Check PDCP-PDCP data forwarding (We should implement similar capability in NR-NSA)
      do {
        ...
        if (msg_p != NULL) {
          switch (ITTI_MSG_ID(msg_p)) {
            case GTPV1U_ENB_DATA_FORWARDING_IND:
              ...
              // PAVE, forwarding data from handover
...
//-----------------------------------------------------------------------------
{
  LOG_A(RRC, "Anyway, RRC connection reconfiguration completed.. Then?\n");
  int                                 drb_id;
  int                                 oip_ifup = 0;
  ...
  LOG_A(RRC, "CDRX on for rnti %d, %d!\n", rnti, ctxt_pP->rntiMaybeUEid);
  ...
  if (SRB_configList != NULL) {
    for (int i = 0; (i < SRB_configList->list.count) && (i < 3); i++) {
      if (SRB_configList->list.array[i]->srb_Identity == 1) {
        ...
        ue_context_pP->ue_context.Srb2.Srb_info.Srb_id = 2;
        LOG_A(RRC,"[eNB %d] Frame  %d CC %d : SRB2 is now active\n",
              ctxt_pP->module_id,
              ctxt_pP->frame,
              ue_context_pP->ue_context.primaryCC_id);
      } else {
        LOG_E(RRC,"[eNB %d] Frame  %d CC %d : invalide SRB identity %ld\n",
              ctxt_pP->module_id,
              ctxt_pP->frame,
              ue_context_pP->ue_context.primaryCC_id,
  ...
  if (DRB_configList != NULL) {
    for (int i = 0; i < DRB_configList->list.count; i++) {  // num max DRB (11-3-8)
      if (DRB_configList->list.array[i]) {
        ...
        /* For pre-ci tests */
        LOG_A(RRC, "[eNB %d] Frame  %d : Logical Channel UL-DCCH, Received LTE_RRCConnectionReconfigurationComplete, reconfiguring DRB %d/LCID %d\n",
              ctxt_pP->module_id,
              ctxt_pP->frame,
              (int)DRB_configList->list.array[i]->drb_Identity,
              (int)*DRB_configList->list.array[i]->logicalChannelIdentity);
        ...
        if (ue_context_pP->ue_context.DRB_active[drb_id] == 0) {
          ...
          LOG_A(RRC, "[eNB %d] Frame %d: Establish RLC UM Bidirectional, DRB %d Active\n",
          ...
          if (!EPC_MODE_ENABLED && !ENB_NAS_USE_TUN) {
            ...
            if (oip_ifup == 0) { // interface is up --> send a config the DRB
              ...
              LOG_A(RRC, "[eNB %d] State = Attached (UE rnti %x module id %u)\n",
  ...
  if(DRB_Release_configList2 != NULL) {
    LOG_A(RRC, "There is DRB release list!");
...
void process_endc_lte_info_indication(protocol_ctxt_t *const ctxt_pP, x2ap_ENDC_lte_info_indication_t *m) {

  MessageDef *msg;
  
  msg = itti_alloc_new_message(TASK_RRC_ENB, 0, X2AP_ENDC_LTE_INFO_INDICATION);
  memset(&(X2AP_ENDC_LTE_INFO_INDICATION(msg)), 0, sizeof(x2ap_ENDC_lte_info_indication_t));

  X2AP_ENDC_LTE_INFO_INDICATION(msg).rnti     = m->rnti;
  X2AP_ENDC_LTE_INFO_INDICATION(msg).rb_id    = m->rb_id;
  X2AP_ENDC_LTE_INFO_INDICATION(msg).sdu_size = m->sdu_size;
  X2AP_ENDC_LTE_INFO_INDICATION(msg).total_buf = m->total_buf;

  //LOG_E(RRC, "module id is: %d", ctxt_pP->module_id);
...
void rrc_eNB_process_AdditionResponseInformation(const module_id_t enb_mod_idP, x2ap_ENDC_sgnb_addition_req_ACK_t *m) {
  ...
  if (ue_context == NULL) {
    ...
    temp_enb_rnti = m->rnti;
    LOG_A(RRC, "ue_context for RNTI %x\n", m->rnti);
  }
  ...
  rrc_data_req(&ctxt, DCCH, rrc_eNB_mui++, SDU_CONFIRM_NO, size, buffer, PDCP_TRANSMISSION_MODE_CONTROL); // PAVE test
}

void print_rrc_data_forwarding_info(const x2ap_ENDC_sgnb_data_forwarding_t *m) {
    if (m == NULL) {
        LOG_E(RRC, "Error: Null pointer passed to print_rrc_data_forwarding_info");
        return;
    }

    LOG_E(RRC, "RRC Data Forwarding Information:");
    LOG_E(RRC, "  MeNB UE X2AP ID: %d", m->MeNB_ue_x2_id);
    LOG_E(RRC, "  SgNB UE X2AP ID: %d", m->SgNB_ue_x2_id);
    LOG_E(RRC, "  gNB X2 assoc ID: %d", m->gnb_x2_assoc_id);
    LOG_E(RRC, "  RNTI: %d", m->rnti);
    
    LOG_E(RRC, "  Last visited cell info:");
    // Assuming x2ap_lastvisitedcell_info_t has relevant fields, adjust as necessary
    // LOG_E(RRC, "    Field1: %d", m->lastvisitedcell_info.field1);
    // LOG_E(RRC, "    Field2: %d", m->lastvisitedcell_info.field2);
    // Add more fields as needed
    
    LOG_E(RRC, "  Target assoc ID: %d", m->target_assoc_id);
    LOG_E(RRC, "  eNB flag: %u", m->enb_flag);
    LOG_E(RRC, "  SRB flag: %d", m->srb_flag);
    LOG_E(RRC, "  RB ID: %ld", m->rb_id);  // Changed to %ld for rb_id_t (long int)
    LOG_E(RRC, "  MUI: %d", m->muip);
    LOG_E(RRC, "  Confirm: %d", m->confirmp);
    LOG_E(RRC, "  SDU size: %u", m->sdu_size);
    LOG_E(RRC, "  PDCP transmission mode: %d", m->mode);
    LOG_E(RRC, "  Source L2 ID: 0x%x", m->sourceL2Id);
    LOG_E(RRC, "  Destination L2 ID: 0x%x", m->destinationL2Id);

    if (m->sdu_p != NULL && m->sdu_size > 0) {
        LOG_E(RRC, "  SDU data present (size: %u bytes)", m->sdu_size);
        LOG_E(RRC, "  SDU data (first 16 bytes or less):");
        for (uint32_t i = 0; i < m->sdu_size && i < 16; i++) {
            printf("%02x ", m->sdu_p[i]);
        }
        printf("\n");
    } else {
        LOG_E(RRC, "  No SDU data present");
    }
}
...
// PAVE
void rrc_eNB_process_DataForwarding(const module_id_t enb_mod_idP, x2ap_ENDC_sgnb_data_forwarding_t *m) {

  //print_rrc_data_forwarding_info(m);
  protocol_ctxt_t ctxt;
  // Check if PROTOCOL_CTXT_SET_BY_INSTANCE macro is available
  PROTOCOL_CTXT_SET_BY_INSTANCE(&ctxt, enb_mod_idP, ENB_FLAG_YES, temp_enb_rnti, 0, 0);

  // Now let's decode the SN from the buffer
  int sn = 0;
  int dc_bit = m->sdu_p[0] & 0x80;  // Check if it's a data/control bit (0x80 bit)

  // Assuming the SN size (you should replace it based on your SN size configuration)
  
  sn = ((m->sdu_p[0] & 0x3) << 16) |
        (m->sdu_p[1]        <<  8) |
        m->sdu_p[2];

  // Print the decoded SN
  //LOG_E(RRC, "Decoded SN is %d\n", sn);
 
  int result = pdcp_data_req(&ctxt,
                             m->srb_flag,  // SRB_FLAG_NO or the actual flag from the message
                             m->rb_id,
                             m->muip,
                             m->confirmp,
                             m->sdu_size,
                             m->sdu_p,
                             3,  // Assuming this corresponds to PDCP_TRANSMISSION_MODE_TRANSPARENT (0:Unknown, 1: Control, 2: Data, 3: TRANSPARENT)
                             &m->sourceL2Id,
                             &m->destinationL2Id
                            );
}
...
void *rrc_enb_process_itti_msg(void *notUsed) {
  ...
  switch (ITTI_MSG_ID(msg_p)) {
    ...
    // PAVE
    case X2AP_ENDC_SGNB_DATA_FORWARDING: {
      rrc_eNB_process_DataForwarding(ENB_INSTANCE_TO_MODULE_ID(instance), &X2AP_ENDC_SGNB_DATA_FORWARDING(msg_p));
      break;
    }
    ...
    case RLC_SDU_INDICATION:
      ...
      break;
    case X2AP_ENDC_LTE_INFO_INDICATION:
        PROTOCOL_CTXT_SET_BY_INSTANCE(&ctxt, instance, ENB_FLAG_YES, X2AP_ENDC_LTE_INFO_INDICATION(msg_p).rnti, 0,0);
        process_endc_lte_info_indication(&ctxt, &X2AP_ENDC_LTE_INFO_INDICATION(msg_p));
      break;
    default:
