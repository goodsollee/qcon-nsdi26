...
/*dual-connectivity lte info*/ //Start from here 0930
static void rrc_process_endc_lte_info(const module_id_t gnb_mod_idP, x2ap_ENDC_lte_info_indication_t *m)
{
  gNB_RRC_INST         *rrc=RC.nrrrc[gnb_mod_idP];
  endc_lte_info_gNB_to_internode(rrc, m);
}
...
void *rrc_gnb_task(void *args_p) {
  ...
  while (1) {
    ...
    switch (ITTI_MSG_ID(msg_p)) {
      ...
      /*dual-connectivity lte info receive*/
      case X2AP_ENDC_LTE_INFO_INDICATION:
        rrc_process_endc_lte_info (instance, &X2AP_ENDC_LTE_INFO_INDICATION(msg_p));
        break;
      default:
