...
void rrc_add_nsa_user(gNB_RRC_INST *rrc, rrc_gNB_ue_context_t *ue_context_p, x2ap_ENDC_sgnb_addition_req_t *m)
{
  ...
  if (get_softmodem_params()->phy_test == 1 || get_softmodem_params()->do_ra == 1 || get_softmodem_params()->sa == 1){
    ...
    //UE->rb_config = get_default_rbconfig(m->e_rabs_tobeadded[0].e_rab_id, m->e_rabs_tobeadded[0].drb_ID, cipher_algo, NR_SecurityConfig__keyToUse_master);
  }
  ...
  LOG_E(RRC, "Assign CU UE ID %d and DU UE ID %d to UE RNTI %04x, LTE RNTI %d\n", rrc_ue_id, du_ue_id, ue_context_p->ue_context.rnti,  (int)(m->rnti));
  ...
  LOG_E(RRC,
        "Calling RRC PDCP/RLC ASN1 request functions for protocol context %p with module_id %d, rnti %d, frame %d, subframe %d eNB_index %d \n",
        &ctxt,
        ctxt.module_id,
        ctxt.rntiMaybeUEid,
        ctxt.frame,
  ...
  // PAVE: add information to PDCP for path routing
  forward_info_t info;
  info.menb_ue_x2_id = m->ue_x2_id;
  info.sgnb_ue_x2_id = secondaryCellGroup->spCellConfig->reconfigurationWithSync->newUE_Identity;
  info.rnti = m->rnti;
  info.assoc_id = m->target_assoc_id;
  info.nr_rnti = du_ue_id;
  info.user = (DC_user *)malloc(sizeof(DC_user));
  initialize_dc_user(info.user, du_ue_id);

  // Add or update the forward info
  add_forward_info(&g_forward_info_map, du_ue_id, &info);
  // TODO: Should be modified for multi-ue feature
  add_forward_info(&g_forward_info_map_lte, 1111, &info);
...
void rrc_remove_nsa_user(gNB_RRC_INST *rrc, int rnti) {
  ...
  // Remove the forward info
  remove_forward_info(&g_forward_info_map, ctxt.rntiMaybeUEid);
...
/*dual-connectivity info*/
void endc_lte_info_internode_to_nsa (gNB_RRC_INST *rrc, x2ap_ENDC_lte_info_indication_t *m) {
  //forward_info_t *info = get_forward_info (&g_forward_info_map_lte, m->rnti);
  // TODO: Currently, LTE rnti is not well delivered when adding sgNB. This should be fixed when we add multi-ue feature.
  forward_info_t *info = get_forward_info (&g_forward_info_map_lte, 1111);
  if (info == NULL)
    return;
  lte_pdcp_dc_info_update ((int)(info->nr_rnti), m->sdu_size, m->total_buf);
}