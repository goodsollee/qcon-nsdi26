...
// PAVE
#define X2AP_ENDC_SGNB_DATA_FORWARDING(mSGpTR)                 (mSGpTR)->ittiMsg.x2ap_ENDC_sgnb_data_forwarding
#define X2AP_ENDC_LTE_INFO_INDICATION(mSGpTR)                  (mSGpTR)->ittiMsg.x2ap_ENDC_lte_info_indication
...
typedef struct x2ap_ENDC_sgnb_addition_req_s {
 ...
 /* list of e_rab to be added by RRC layers */
  ...
  /*Dual-connectivity X2U interface*/
  transport_layer_addr_t eNB_addr [S1AP_MAX_E_RAB];
...
typedef struct x2ap_ENDC_sgnb_data_forwarding_s {

  // Fowarding information
  int MeNB_ue_x2_id;
...
} x2ap_ENDC_sgnb_data_forwarding_t;
...
// eNB: RLC -> NR PDCP messages
typedef struct x2ap_ENDC_lte_info_indication_s {
  int rnti;
  int rb_id;
  int sdu_size;
  int total_buf;
} x2ap_ENDC_lte_info_indication_t;
