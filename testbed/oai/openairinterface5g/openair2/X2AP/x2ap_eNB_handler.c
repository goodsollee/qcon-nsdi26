...
// PAVE
int x2ap_eNB_handle_receive_data_forwarding (instance_t instance,
                            sctp_assoc_t assoc_id,
                            int32_t stream,
                            const uint8_t *const data,
                            const uint32_t data_length);
// PAVE (DC)
int x2ap_eNB_handle_receive_endc_lte_info(instance_t instance, void* zmq_context);                         
...
// PAVE decode data
// TODO: add rnti with id manager 
x2ap_ENDC_sgnb_data_forwarding_t* x2ap_gNB_decode_data_forwarding(uint8_t *buffer, size_t buffer_size) {
    if (buffer == NULL || buffer_size == 0) {
        return NULL;
    }

    x2ap_ENDC_sgnb_data_forwarding_t *decoded = malloc(sizeof(x2ap_ENDC_sgnb_data_forwarding_t));
    if (decoded == NULL) {
        return NULL;
    }
    ...
    uint8_t *ptr = buffer;
     ...
     // Decode integer fields
    #define DECODE_INT(field) do { \
        int32_t netvalue; \
        memcpy(&netvalue, ptr, sizeof(int32_t)); \
        decoded->field = ntohl(netvalue); \
        ptr += sizeof(int32_t); \
    } while(0)

    #define DECODE_DOUBLE(field) do { \
        double netvalue; \
        memcpy(&netvalue, ptr, sizeof(double)); \
        decoded->field = netvalue; \
        ptr += sizeof(double); \
    } while(0)

    DECODE_INT(MeNB_ue_x2_id);
    DECODE_INT(SgNB_ue_x2_id);
    DECODE_INT(gnb_x2_assoc_id);
    DECODE_INT(rnti);

    // Decode lastvisitedcell_info
    memcpy(&decoded->lastvisitedcell_info, ptr, sizeof(x2ap_lastvisitedcell_info_t));
    ptr += sizeof(x2ap_lastvisitedcell_info_t);

    DECODE_INT(target_assoc_id);

    // Decode the rest of the fields
    decoded->enb_flag = *ptr++;
    memcpy(&decoded->srb_flag, ptr, sizeof(srb_flag_t));
    ptr += sizeof(srb_flag_t);
    memcpy(&decoded->rb_id, ptr, sizeof(rb_id_t));
    ptr += sizeof(rb_id_t);
    memcpy(&decoded->muip, ptr, sizeof(mui_t));
    ptr += sizeof(mui_t);
    memcpy(&decoded->confirmp, ptr, sizeof(confirm_t));
    ptr += sizeof(confirm_t);

    uint32_t net_sdu_size;
    memcpy(&net_sdu_size, ptr, sizeof(uint32_t));
    decoded->sdu_size = ntohl(net_sdu_size);
    ptr += sizeof(uint32_t);

    memcpy(&decoded->mode, ptr, sizeof(pdcp_transmission_mode_t));
    ptr += sizeof(pdcp_transmission_mode_t);

    uint32_t net_sourceL2Id;
    memcpy(&net_sourceL2Id, ptr, sizeof(uint32_t));
    decoded->sourceL2Id = ntohl(net_sourceL2Id);
    ptr += sizeof(uint32_t);

    uint32_t net_destinationL2Id;
    memcpy(&net_destinationL2Id, ptr, sizeof(uint32_t));
    decoded->destinationL2Id = ntohl(net_destinationL2Id);
    ptr += sizeof(uint32_t);

    // Decode double timestamp (8 bytes)
    DECODE_DOUBLE(timestamp);

    // Decode SDU data if present
    size_t remaining_size = buffer_size - (ptr - buffer);
    if (remaining_size > 0 && decoded->sdu_size > 0) {
        decoded->sdu_p = malloc(decoded->sdu_size);
        if (decoded->sdu_p == NULL) {
            free(decoded);
            return NULL;
        }
        memcpy(decoded->sdu_p, ptr, decoded->sdu_size);
    } else {
        decoded->sdu_p = NULL;
    }
...
void x2ap_gNB_free_decoded_data_forwarding(x2ap_ENDC_sgnb_data_forwarding_t *decoded) {
    if (decoded) {
        if (decoded->sdu_p) {
            free(decoded->sdu_p);
        }
                                                   ...
                                                   const uint32_t data_length)
{
  ...
  //LOG_A(X2AP, "[%ld.%09ld] Starting x2ap_eNB_handle_receive_data_forwarding\n", 
  //           start.tv_sec, start.tv_nsec);

  x2ap_eNB_instance_t *instance_p;
  MessageDef *msg;
  x2ap_ENDC_sgnb_data_forwarding_t *decoded_data;

  instance_p = x2ap_eNB_get_instance(instance);
  DevAssert(instance_p != NULL);
  
  // Decode the received data
  decoded_data = x2ap_gNB_decode_data_forwarding((uint8_t *)data, data_length);
  if (decoded_data == NULL) {
    X2AP_ERROR("Failed to decode received data forwarding message\n");
    return -1;
  }

  // Generate ITTI message
  msg = itti_alloc_new_message(TASK_X2AP, 0, X2AP_ENDC_SGNB_DATA_FORWARDING);
  DevAssert(msg != NULL);

  // Copy decoded data to the message
  X2AP_ENDC_SGNB_DATA_FORWARDING(msg).MeNB_ue_x2_id = decoded_data->MeNB_ue_x2_id;
  X2AP_ENDC_SGNB_DATA_FORWARDING(msg).SgNB_ue_x2_id = decoded_data->SgNB_ue_x2_id;
  X2AP_ENDC_SGNB_DATA_FORWARDING(msg).gnb_x2_assoc_id = decoded_data->gnb_x2_assoc_id;
  X2AP_ENDC_SGNB_DATA_FORWARDING(msg).rnti =  x2ap_id_get_rnti(&instance_p->id_manager, decoded_data->MeNB_ue_x2_id);
  memcpy(&X2AP_ENDC_SGNB_DATA_FORWARDING(msg).lastvisitedcell_info, &decoded_data->lastvisitedcell_info, sizeof(x2ap_lastvisitedcell_info_t));
  X2AP_ENDC_SGNB_DATA_FORWARDING(msg).target_assoc_id = decoded_data->target_assoc_id;
  
  X2AP_ENDC_SGNB_DATA_FORWARDING(msg).enb_flag = decoded_data->enb_flag;
  X2AP_ENDC_SGNB_DATA_FORWARDING(msg).srb_flag = decoded_data->srb_flag;
  X2AP_ENDC_SGNB_DATA_FORWARDING(msg).rb_id = decoded_data->rb_id;
  X2AP_ENDC_SGNB_DATA_FORWARDING(msg).muip = decoded_data->muip;
  X2AP_ENDC_SGNB_DATA_FORWARDING(msg).confirmp = decoded_data->confirmp;
  X2AP_ENDC_SGNB_DATA_FORWARDING(msg).sdu_size = decoded_data->sdu_size;
  X2AP_ENDC_SGNB_DATA_FORWARDING(msg).mode = decoded_data->mode;
  X2AP_ENDC_SGNB_DATA_FORWARDING(msg).sourceL2Id = decoded_data->sourceL2Id;
  X2AP_ENDC_SGNB_DATA_FORWARDING(msg).destinationL2Id = decoded_data->destinationL2Id;
  X2AP_ENDC_SGNB_DATA_FORWARDING(msg).timestamp = decoded_data->timestamp;

  // Copy SDU data if present
  if (decoded_data->sdu_p && decoded_data->sdu_size > 0) {
    X2AP_ENDC_SGNB_DATA_FORWARDING(msg).sdu_p = malloc(decoded_data->sdu_size);
    if (X2AP_ENDC_SGNB_DATA_FORWARDING(msg).sdu_p == NULL) {
      X2AP_ERROR("Failed to allocate memory for SDU data\n");
      x2ap_gNB_free_decoded_data_forwarding(decoded_data);
      itti_free(ITTI_MSG_ORIGIN_ID(msg), msg);
      return -1;
    }
    memcpy(X2AP_ENDC_SGNB_DATA_FORWARDING(msg).sdu_p, decoded_data->sdu_p, decoded_data->sdu_size);
  } else {
    X2AP_ENDC_SGNB_DATA_FORWARDING(msg).sdu_p = NULL;
  }

  // Send the message to the RRC task
  ...
  clock_gettime(CLOCK_REALTIME, &currentTime); // CLOCK_MONOTONIC is used for steady time
  double end_time = (double)currentTime.tv_sec * 1000000 + currentTime.tv_nsec / 1000;
  // Calculate the time taken in nanoseconds
  double time_consumed = end_time - decoded_data->timestamp;

  // Print the execution time
  //printf("SCTP communication time: %f us, %f, %f, %d\n", time_consumed, decoded_data->timestamp, end_time, decoded_data->SgNB_ue_x2_id);
...
// Decode function
x2ap_ENDC_lte_info_indication_t* x2ap_gNB_decode_endc_lte_info(uint8_t *buffer, size_t buffer_size) {
    if (buffer == NULL || buffer_size == 0) {
        return NULL;
    }

    x2ap_ENDC_lte_info_indication_t *decoded = malloc(sizeof(x2ap_ENDC_lte_info_indication_t));
    if (decoded == NULL) {
        return NULL;
    }
    ...
    DECODE_INT(rb_id);
    DECODE_INT(sdu_size);
    DECODE_INT(total_buf);

    return decoded;
}

// Free function for decoded data
void x2ap_gNB_free_decoded_endc_lte_info(x2ap_ENDC_lte_info_indication_t *decoded) {
    if (decoded) {
        free(decoded);
    }
}
...
// Print function for LTE INFO indication
void x2ap_gNB_print_endc_lte_info(const x2ap_ENDC_lte_info_indication_t *info) {
    if (info == NULL) {
        X2AP_ERROR("Cannot print NULL LTE INFO indication\n");
        return;
    }
}


// Receive, decode, and forward function
int x2ap_gNB_handle_receive_endc_lte_info(instance_t instance, void* data, size_t data_length)
{
    struct timespec start, end;
    clock_gettime(CLOCK_REALTIME, &start);
    ...
    x2ap_eNB_instance_t *instance_p;
    MessageDef *msg;
    x2ap_ENDC_lte_info_indication_t *decoded_data;

    instance_p = x2ap_eNB_get_instance(instance);
    DevAssert(instance_p != NULL);
  
    // Decode the received data
    decoded_data = x2ap_gNB_decode_endc_lte_info(data, data_length);
    if (decoded_data == NULL) {
        X2AP_ERROR("Failed to decode received ENDC LTE info message\n");
        return -1;
    }

    // Print the decoded LTE INFO indication
    x2ap_gNB_print_endc_lte_info(decoded_data);

    // Generate ITTI message
    msg = itti_alloc_new_message(TASK_X2AP, 0, X2AP_ENDC_LTE_INFO_INDICATION);
    DevAssert(msg != NULL);

    // Copy decoded data to the message
    X2AP_ENDC_LTE_INFO_INDICATION(msg).rnti = decoded_data->rnti;
    X2AP_ENDC_LTE_INFO_INDICATION(msg).rb_id = decoded_data->rb_id;
    X2AP_ENDC_LTE_INFO_INDICATION(msg).sdu_size = decoded_data->sdu_size;
    X2AP_ENDC_LTE_INFO_INDICATION(msg).total_buf = decoded_data->total_buf;

    // Send the message to the RRC task
    itti_send_msg_to_task(TASK_RRC_GNB, instance_p->instance, msg);

    // Free the decoded data
    x2ap_gNB_free_decoded_endc_lte_info(decoded_data);

    clock_gettime(CLOCK_REALTIME, &end);
    double time_consumed = (end.tv_sec - start.tv_sec) * 1e6 + (end.tv_nsec - start.tv_nsec) / 1e3;
    
    // Print the execution time
    X2AP_DEBUG("ENDC LTE info handling time: %.3f us\n", time_consumed);
                                                      ...
                                                      X2AP_X2AP_PDU_t *pdu)
{
  ...
  printf("x2ap gNB handle senb addtion request!!");
