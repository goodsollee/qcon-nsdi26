...
static
void x2ap_eNB_handle_endc_lte_info_ind(instance_t instance, x2ap_ENDC_lte_info_indication_t *x2ap_endc_lte_info_ind);
...
static
void x2ap_eNB_handle_sctp_data_forward(instance_t instance, sctp_data_forward_t *sctp_data_forward) {
  int result;
  DevAssert(sctp_data_forward != NULL);
  x2ap_eNB_handle_receive_data_forwarding(instance, sctp_data_forward->assoc_id, sctp_data_forward->stream,
                          sctp_data_forward->buffer, sctp_data_forward->buffer_length);
  result = itti_free(TASK_UNKNOWN, sctp_data_forward->buffer);
  AssertFatal (result == EXIT_SUCCESS, "Failed to free memory (%d)!\n", result);
}
...
int x2ap_eNB_init_sctp (x2ap_eNB_instance_t *instance_p,
  ...
  if (!is_gnb) { // 5G NSA gNB (data sender in downlink)
    // Print IP address and port for debugging
    LOG_E(X2AP, "eNB init SCTP, IP addr: %s, Port: %u\n", 
        local_ip_addr->ipv4_address, enb_port_for_X2C);
    x2ap_zmq_init (local_ip_addr->ipv4_address, local_ip_addr->ipv4_address);
  }
...
static void x2ap_eNB_register_eNB(x2ap_eNB_instance_t *instance_p,
  ...
  LOG_E(X2AP, "Target addr: %s, Local addr: %s", target_eNB_ip_address, local_ip_addr);

  // X2U
  x2ap_zmq_init (target_eNB_ip_address, local_ip_addr);
...
static
void x2ap_gNB_sgNB_data_forwarding(instance_t instance,
		x2ap_ENDC_sgnb_data_forwarding_t *x2ap_ENDC_sgnb_data_forwarding)
{
  /* TODO: remove this hack (the goal is to find the correct
   * eNodeB structure for the other end) - we need a proper way for RRC
   * and X2AP to identify eNodeBs
  ...
  instance_p = x2ap_eNB_get_instance(instance);
  DevAssert(instance_p != NULL);
  target = x2ap_get_eNB(NULL, x2ap_ENDC_sgnb_data_forwarding->target_assoc_id, 0);
  DevAssert(target != NULL);
  ...
  x2ap_set_ids(id_manager, ue_id,
      x2ap_ENDC_sgnb_data_forwarding->SgNB_ue_x2_id,
      x2ap_ENDC_sgnb_data_forwarding->MeNB_ue_x2_id,
      x2ap_ENDC_sgnb_data_forwarding->SgNB_ue_x2_id);
  x2ap_id_set_state(id_manager, ue_id, X2ID_STATE_NSA_GNB_OVERALL);
  x2ap_set_dc_overall_timer(id_manager, ue_id,
                            x2ap_timer_get_tti(&instance_p->timers));
  ...
  */
  ...
  x2ap_gNB_generate_data_forwarding(instance_p, target,
      x2ap_ENDC_sgnb_data_forwarding);
}
...
// Here to start.. we need to send lte control info to NR!
static
void x2ap_eNB_handle_endc_lte_info_ind(instance_t instance, x2ap_ENDC_lte_info_indication_t *x2ap_endc_lte_info_ind)
{
  x2ap_eNB_instance_t *instance_p;
  ...
  x2ap_eNB_generate_endc_lte_info(instance_p, x2ap_endc_lte_info_ind);
}
  ...
  while (1) {
    ...
    switch (ITTI_MSG_ID(received_msg)) {
      ...
      case X2AP_ENDC_SGNB_DATA_FORWARDING:
        x2ap_gNB_sgNB_data_forwarding (ITTI_MSG_DESTINATION_INSTANCE(received_msg),
    			  &X2AP_ENDC_SGNB_DATA_FORWARDING(received_msg));
        break;
      ...
      case SCTP_NEW_ASSOCIATION_IND:
        ...
        break;
      case SCTP_DATA_FORWARD: 
        // Handle the SCTP data forwarding
        x2ap_eNB_handle_sctp_data_forward(ITTI_MSG_DESTINATION_INSTANCE(received_msg),
                                      &received_msg->ittiMsg.sctp_data_forward);
        break;
      case SCTP_DATA_IND:
        x2ap_eNB_handle_sctp_data_ind(ITTI_MSG_DESTINATION_INSTANCE(received_msg),
                                      &received_msg->ittiMsg.sctp_data_ind);
        break;
      // LTE dual-connectivity control info from LTE to NR
      case X2AP_ENDC_LTE_INFO_INDICATION:
        x2ap_eNB_handle_endc_lte_info_ind(ITTI_MSG_DESTINATION_INSTANCE(received_msg),
                                      &X2AP_ENDC_LTE_INFO_INDICATION(received_msg));
        break;
      default:
...
// x2ap_eNB.c
void *x2ap_zmq_send_socket = NULL;
void *x2ap_zmq_recv_socket = NULL;
void *x2ap_zmq_context = NULL;
static pthread_t recv_thread;
static volatile int keep_running = 1;

void* x2ap_zmq_recv_thread(void* arg) {
    zmq_pollitem_t items[] = {
        { x2ap_zmq_recv_socket, 0, ZMQ_POLLIN, 0 }
    };

    while (keep_running) {
        // Poll the socket with a 100ms timeout (adjustable as needed)
        ...
        if (items[0].revents & ZMQ_POLLIN) {
            ...
            int recv_result = zmq_msg_recv(&message, x2ap_zmq_recv_socket, ZMQ_DONTWAIT);
            if (recv_result == -1) {
            ...
            intptr_t instance;
            memcpy(&instance, data, sizeof(intptr_t));

            // Calculate the size of the remaining data
            size_t remaining_size = size - sizeof(intptr_t);
            void* remaining_data = (char*)data + sizeof(intptr_t);

            //printf("Received a packet! Instance: %ld, Remaining data size: %zu\n", (long)instance, remaining_size);

            // Call x2ap_eNB_handle_receive_data_forwarding
            // Note: We're passing 0 for assoc_id and stream as they're not included in the ZeroMQ message
            if (is_gnb) { // NR receive
              x2ap_gNB_handle_receive_endc_lte_info(instance, remaining_data, remaining_size);
            }
            else { // LTE receive
              x2ap_eNB_handle_receive_data_forwarding(instance, 0, 0, remaining_data, remaining_size);
            }

            zmq_msg_close(&message);
        }
        // You can add more logic here if necessary, such as additional polling or other tasks
    }
...
void x2ap_zmq_init(const char* send_ip_addr, const char* recv_ip_addr) {
    ...
    x2ap_zmq_context = zmq_ctx_new();
    if (!x2ap_zmq_context) {
        perror("Failed to create ZeroMQ context");
        exit(1);
    }

    int hwm = 10000;  // High Water Mark (HWM) to buffer up to 10,000 messages
    int linger = 0;   // Set linger to 0 for immediate socket closure

    char send_endpoint[64];
    char recv_endpoint[64];

    if (is_gnb) {
        // Create and connect send (PUSH) socket to port 5555, receive on port 5556
        x2ap_zmq_send_socket = zmq_socket(x2ap_zmq_context, ZMQ_PUSH);
        if (!x2ap_zmq_send_socket) {
            perror("Failed to create ZeroMQ send socket");
            zmq_ctx_destroy(x2ap_zmq_context);
            exit(1);
        }

        // Set send socket options
        zmq_setsockopt(x2ap_zmq_send_socket, ZMQ_SNDHWM, &hwm, sizeof(hwm));
        zmq_setsockopt(x2ap_zmq_send_socket, ZMQ_LINGER, &linger, sizeof(linger));

        snprintf(send_endpoint, sizeof(send_endpoint), "tcp://localhost:5555", send_ip_addr);
        if (zmq_connect(x2ap_zmq_send_socket, send_endpoint) != 0) {
            perror("Failed to connect ZeroMQ send socket");
            zmq_close(x2ap_zmq_send_socket);
            zmq_ctx_destroy(x2ap_zmq_context);
            exit(1);
        }

        // Create and bind receive (PULL) socket on port 5556
        x2ap_zmq_recv_socket = zmq_socket(x2ap_zmq_context, ZMQ_PULL);
        if (!x2ap_zmq_recv_socket) {
            perror("Failed to create ZeroMQ receive socket");
            zmq_ctx_destroy(x2ap_zmq_context);
            exit(1);
        }

        // Set receive socket options
        zmq_setsockopt(x2ap_zmq_recv_socket, ZMQ_RCVHWM, &hwm, sizeof(hwm));
        zmq_setsockopt(x2ap_zmq_recv_socket, ZMQ_LINGER, &linger, sizeof(linger));

        snprintf(recv_endpoint, sizeof(recv_endpoint), "tcp://localhost:5556", recv_ip_addr);
        if (zmq_bind(x2ap_zmq_recv_socket, recv_endpoint) != 0) {
            perror("Failed to bind ZeroMQ receive socket");
            zmq_close(x2ap_zmq_recv_socket);
            zmq_ctx_destroy(x2ap_zmq_context);
            exit(1);
        }

        // Create and start the receive thread
        if (pthread_create(&recv_thread, NULL, x2ap_zmq_recv_thread, NULL) != 0) {
        ...
        // Create and connect send (PUSH) socket to port 5556, receive on port 5555
        x2ap_zmq_send_socket = zmq_socket(x2ap_zmq_context, ZMQ_PUSH);
        if (!x2ap_zmq_send_socket) {
            perror("Failed to create ZeroMQ send socket");
            zmq_ctx_destroy(x2ap_zmq_context);
        ...
        snprintf(send_endpoint, sizeof(send_endpoint), "tcp://localhost:5556", send_ip_addr);
        if (zmq_connect(x2ap_zmq_send_socket, send_endpoint) != 0) {
            perror("Failed to connect ZeroMQ send socket");
            zmq_close(x2ap_zmq_send_socket);
            zmq_ctx_destroy(x2ap_zmq_context);
        ...
        // Create and bind receive (PULL) socket on port 5555
        x2ap_zmq_recv_socket = zmq_socket(x2ap_zmq_context, ZMQ_PULL);
        if (!x2ap_zmq_recv_socket) {
            perror("Failed to create ZeroMQ receive socket");
            zmq_ctx_destroy(x2ap_zmq_context);
        ...
        snprintf(recv_endpoint, sizeof(recv_endpoint), "tcp://localhost:5555", recv_ip_addr);
        if (zmq_bind(x2ap_zmq_recv_socket, recv_endpoint) != 0) {
            perror("Failed to bind ZeroMQ receive socket");
            zmq_close(x2ap_zmq_recv_socket);
            zmq_ctx_destroy(x2ap_zmq_context);
...
void x2ap_zmq_send(const char *data) {
    zmq_send(x2ap_zmq_send_socket, data, strlen(data), 0);
}

// Function to receive data
void x2ap_zmq_recv(char *buffer, size_t buffer_size) {
    zmq_recv(x2ap_zmq_recv_socket, buffer, buffer_size, 0);
}