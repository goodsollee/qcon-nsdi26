...
int x2ap_gNB_generate_data_forwarding(x2ap_eNB_instance_t *instance_p, x2ap_eNB_data_t *x2ap_eNB_data_p,
                                      x2ap_ENDC_sgnb_data_forwarding_t *x2ap_data_forwarding)
{
    if (!instance_p || !x2ap_eNB_data_p || !x2ap_data_forwarding) {
        LOG_E(X2AP, "Invalid input parameters (NR data forwarding)");
        return -1;
    }

    // Calculate buffer size
    size_t buffer_size = sizeof(intptr_t) + // instance id
                         sizeof(int) * 6 +  // For the first 6 int fields
                         sizeof(x2ap_lastvisitedcell_info_t) +
                         sizeof(uint8_t) + sizeof(srb_flag_t) + sizeof(rb_id_t) +
                         sizeof(mui_t) + sizeof(confirm_t) + sizeof(sdu_size_t) +
                         sizeof(pdcp_transmission_mode_t) + sizeof(uint32_t) * 2 +
                         x2ap_data_forwarding->sdu_size + sizeof(double);  // Add size for timestamp

    // Allocate buffer
    uint8_t *buffer = (uint8_t *)malloc(buffer_size);
    if (!buffer) {
        LOG_E(X2AP, "Memory allocation failed");
        return -1;
    }
    ...
    // Macro to safely copy data
    #define SAFE_COPY(dst, src, size) do { \
        if (dst + size <= buffer_end) { \
            memcpy(dst, src, size); \
            dst += size; \
        } else { \
            LOG_E(X2AP, "Buffer overflow prevented"); \
            free(buffer); \
            return -1; \
        } \
    } while(0)
    ...
    // Encode each field
    #define ENCODE_INT(value) do { \
        int32_t netvalue = htonl(value); \
        SAFE_COPY(ptr, &netvalue, sizeof(int32_t)); \
    } while(0)

    #define ENCODE_DOUBLE(value) do { \
        SAFE_COPY(ptr, &value, sizeof(double)); \
    } while(0)

    // Encode instance
    intptr_t net_instance = instance_p->instance;
    // Convert to network byte order if necessary (see note below)
    memcpy(ptr, &net_instance, sizeof(intptr_t));
    ptr += sizeof(intptr_t);

    ENCODE_INT(x2ap_data_forwarding->MeNB_ue_x2_id);
    ENCODE_INT(x2ap_data_forwarding->SgNB_ue_x2_id);
    ENCODE_INT(x2ap_data_forwarding->gnb_x2_assoc_id);
    ENCODE_INT(x2ap_data_forwarding->rnti);
    
    SAFE_COPY(ptr, &x2ap_data_forwarding->lastvisitedcell_info, sizeof(x2ap_lastvisitedcell_info_t));

    ENCODE_INT(x2ap_data_forwarding->target_assoc_id);

    // Encode the rest of the fields
    if (ptr < buffer_end) *ptr++ = x2ap_data_forwarding->enb_flag;
    SAFE_COPY(ptr, &x2ap_data_forwarding->srb_flag, sizeof(srb_flag_t));
    SAFE_COPY(ptr, &x2ap_data_forwarding->rb_id, sizeof(rb_id_t));
    SAFE_COPY(ptr, &x2ap_data_forwarding->muip, sizeof(mui_t));
    SAFE_COPY(ptr, &x2ap_data_forwarding->confirmp, sizeof(confirm_t));
    
    uint32_t net_sdu_size = htonl(x2ap_data_forwarding->sdu_size);
    SAFE_COPY(ptr, &net_sdu_size, sizeof(uint32_t));

    SAFE_COPY(ptr, &x2ap_data_forwarding->mode, sizeof(pdcp_transmission_mode_t));

    uint32_t net_sourceL2Id = htonl(x2ap_data_forwarding->sourceL2Id);
    SAFE_COPY(ptr, &net_sourceL2Id, sizeof(uint32_t));

    uint32_t net_destinationL2Id = htonl(x2ap_data_forwarding->destinationL2Id);
    SAFE_COPY(ptr, &net_destinationL2Id, sizeof(uint32_t));

    // Encode timestamp
    ENCODE_DOUBLE(x2ap_data_forwarding->timestamp);

    // Copy SDU data if present
    if (x2ap_data_forwarding->sdu_p && x2ap_data_forwarding->sdu_size > 0) {
        SAFE_COPY(ptr, x2ap_data_forwarding->sdu_p, x2ap_data_forwarding->sdu_size);
    }

    // Calculate actual length of encoded data
    size_t len = ptr - buffer;

    // Send the encoded data  (w/ SCTP..need optimization)
    //x2ap_eNB_itti_send_sctp_data_forward(instance_p->instance, x2ap_eNB_data_p->assoc_id, buffer, len, 1);

    // Send the data using ZMQ
    zmq_msg_t msg;
    zmq_msg_init_size(&msg, buffer_size);
    memcpy(zmq_msg_data(&msg), buffer, buffer_size);
    
    int rc = zmq_msg_send(&msg, x2ap_zmq_send_socket, 0);
    if (rc == -1) {
        LOG_E(X2AP, "Failed to send message: %s", zmq_strerror(errno));
        zmq_msg_close(&msg);
        free(buffer);
        return -1;
    }
    ...
    zmq_msg_close(&msg);
    LOG_I(X2AP, "Data forwarding message sent via ZeroMQ, size: %zu bytes", buffer_size);

    return 1;
}

int x2ap_eNB_generate_endc_lte_info(x2ap_eNB_instance_t *instance_p, x2ap_ENDC_lte_info_indication_t *x2ap_endc_lte_info_ind)
{
    if (!instance_p) {
        LOG_E(X2AP, "Invalid input parameters (LTE control info-instance)\n");
        return -1;
    }
    if (!x2ap_endc_lte_info_ind) {
        LOG_E(X2AP, "Invalid input parameters (LTE control info-msg)\n");
        return -1;
    }

    // Calculate buffer size
    size_t buffer_size = sizeof(intptr_t) + // instance id
                         sizeof(int) * 4;   // For the 4 int fields in x2ap_ENDC_lte_info_indication_t
    ...
    // Macro to encode integer values
    #define ENCODE_INT(value) do { \
        int32_t netvalue = htonl(value); \
        SAFE_COPY(ptr, &netvalue, sizeof(int32_t)); \
    } while(0)
    ...
    // Encode each field
    ENCODE_INT(x2ap_endc_lte_info_ind->rnti);
    ENCODE_INT(x2ap_endc_lte_info_ind->rb_id);
    ENCODE_INT(x2ap_endc_lte_info_ind->sdu_size);
    ENCODE_INT(x2ap_endc_lte_info_ind->total_buf);
...
/*
// eNB --> gNB
int x2ap_eNB_generate_ENDC_x2_data_forwarding(
  x2ap_eNB_instance_t *instance_p, x2ap_eNB_data_t *x2ap_eNB_data_p,
  int x2_id_source, int x2_id_target, x2ap_cause_t cause)
{
  return 0;
...
// gNB --> eNB
int x2ap_gNB_generate_ENDC_x2_data_forwarding(
  x2ap_eNB_instance_t *instance_p, x2ap_eNB_data_t *x2ap_eNB_data_p,
  int menb_ue_x2ap_id, int sgnb_ue_x2ap_id)
{
  X2AP_X2AP_PDU_t               pdu;
  X2AP_DataForwardingAddressIndication_t     *out;
  X2AP_DataForwardingAddressIndication_IEs_t *ie;
  ...
  // PAVE: added and modified
  memset(&pdu, 0, sizeof(pdu));
  pdu.present = X2AP_X2AP_PDU_PR_successfulOutcome;
  pdu.choice.successfulOutcome.procedureCode = X2AP_ProcedureCode_id_dataForwardingAddressIndication; // PAVE defined
  pdu.choice.successfulOutcome.criticality = X2AP_Criticality_reject;
  pdu.choice.successfulOutcome.value.present = X2AP_InitiatingMessage__value_PR_DataForwardingAddressIndication; // PAVE defined
  out = &pdu.choice.successfulOutcome.value.choice.DataForwardingAddressIndication; // PAVE defined

  ie = (X2AP_DataForwardingAddressIndication_IEs_t *)calloc(1, sizeof(X2AP_DataForwardingAddressIndication_IEs_t));
  ie->id = X2AP_ProtocolIE_ID_id_MeNB_UE_X2AP_ID;
  ie->criticality= X2AP_Criticality_ignore;
  ie->value.present = X2AP_DataForwardingAddressIndication_IEs__value_PR_UE_X2AP_ID;
  ie->value.choice.UE_X2AP_ID = menb_ue_x2ap_id;
  asn1cSeqAdd(&out->protocolIEs.list, ie);
  ...
  if (x2ap_eNB_encode_pdu(&pdu, &buffer, &len) < 0) {
    X2AP_ERROR("Failed to encode ENDC X2 gNB data forwarding\n");
    return -1;
  }
