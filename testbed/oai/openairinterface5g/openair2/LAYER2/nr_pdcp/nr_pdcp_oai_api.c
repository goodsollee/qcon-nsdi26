...
#include "nr_pdcp_ran_func.h" /* QCON: init_ran_function + deliver_info_to_ric */
...
#include "openair2/LAYER2/NR_MAC_gNB/nr_mac_gNB.h" /* QCON Stage 6b: MAC scheduler state */
#include "openair2/LAYER2/NR_MAC_COMMON/nr_mac_common.h" /* nr_get_Qm_dl, nr_get_code_rate_dl */
...
void nr_pdcp_layer_init(bool uses_e1)
{
  ...
  /* QCON Stage 7: bring up the RIC ZMQ link.
   * init_ran_function() opens DEALER socket to ZMQ_ENDPOINT and starts the
   * recv thread. Failure is non-fatal — gNB still runs without RIC. */
  if (!init_ran_function()) {
    LOG_W(PDCP, "QCON: RAN function init failed; continuing without RIC link\n");
  } else {
    LOG_I(PDCP, "QCON: RAN function up — RIC link at " ZMQ_ENDPOINT "\n");
  }

  LOG_E(PDCP, "NR PDCP initialized!\n");
}
                                ...
                                char *buf, int size, int sdu_id)
{
  ...
  if (NODE_IS_CU(node_type)) {
    ...
    forward_info_t *info = get_forward_info(&g_forward_info_map, ctxt.rntiMaybeUEid);

    LOG_D(PDCP, "DL transmission for ue %d\n", ctxt.rntiMaybeUEid);

    /* QCON Stage 8: route via target_split_ratio. Falls back to NR-only
     * when DC_user is not yet attached (info missing or user pointer NULL),
     * which preserves the pre-DC behavior during attach race. */
    bool is_lte = false;
    if (info != NULL && info->user != NULL && info->user->is_active) {
      is_lte = pdcp_multi_path_routing(info->user);
    }
    bool is_nr = !is_lte;
    if (is_lte) {

      MessageDef *msg;
      msg = itti_alloc_new_message(TASK_RRC_ENB, 0, X2AP_ENDC_SGNB_DATA_FORWARDING);

      if (info != NULL) {
          X2AP_ENDC_SGNB_DATA_FORWARDING(msg).MeNB_ue_x2_id = info->menb_ue_x2_id;
          X2AP_ENDC_SGNB_DATA_FORWARDING(msg).SgNB_ue_x2_id = info->sgnb_ue_x2_id;
          X2AP_ENDC_SGNB_DATA_FORWARDING(msg).rnti = info->rnti;
          X2AP_ENDC_SGNB_DATA_FORWARDING(msg).target_assoc_id = info->assoc_id;

          // Data information section
          //X2AP_ENDC_SGNB_DATA_FORWARDING(msg).enb_flag = ctxt_pP->enb_flag;
          //X2AP_ENDC_SGNB_DATA_FORWARDING(msg).srb_flag = srb_flagP;

          uint8_t *memblock = malloc16(size);
          memcpy(memblock, buf, size);

          X2AP_ENDC_SGNB_DATA_FORWARDING(msg).rb_id = rb_id;
          X2AP_ENDC_SGNB_DATA_FORWARDING(msg).sdu_size = size; //sdu_buffer_size;
          X2AP_ENDC_SGNB_DATA_FORWARDING(msg).sdu_p = memblock; //sdu_buffer;

          struct timespec start, end;
          clock_gettime(CLOCK_REALTIME, &start);

          // Convert to microseconds
          double timestamp_us = (double)start.tv_sec * 1000000 + start.tv_nsec / 1000;
          X2AP_ENDC_SGNB_DATA_FORWARDING(msg).timestamp = timestamp_us;

          // Send the message to the X2AP task
          itti_send_msg_to_task(TASK_X2AP, ENB_MODULE_ID_TO_INSTANCE(0), msg);
          //LOG_E(PDCP, "Forward packet from 5G NR PDCP to X2AP\n");
      }
    }
    if (is_nr) {
      uint8_t *memblock = malloc16(size);
...
// Function to initialize a DC_user instance
void initialize_dc_user(DC_user *user, int user_id) {
    LOG_E(PDCP, "DC user add start! %x\n", (unsigned int)user_id);
    user->user_id = user_id;
    user->ewma_lte_throughput = 0.0;
    user->ewma_nr_throughput = 0.0;
    user->lte_queue_size = 0.0;
    user->nr_queue_size = 0.0;
    user->target_split_ratio = 0.0;  // Initialize to 0

    LOG_E(PDCP, "DC user set memory for user! %x\n", (unsigned int)user_id);

    // Initialize routing history to false (all packets routed to NR by default)
    memset(user->routing_history, false, sizeof(user->routing_history));

    user->routing_index = 0;  // Start at the beginning of the routing history
    user->use_dc = false;     // Dual connectivity is off initially

    pthread_mutex_init(&user->mutex, NULL);
    user->is_active = true;
    user->lte_packets = 0;
    user->nr_packets = 0;
    LOG_E(PDCP, "DC user added! %x\n", (unsigned int)user_id);

    start_ewma_update_thread(user);

    load_config();

    /* QCON Stage 6: register this DC_user with the RIC publisher so the
     * KPM thread will start emitting per-UE throughput. */
    register_dc_user_with_ric(user);
}

forward_info_map_t g_forward_info_map = {0};
forward_info_map_t g_forward_info_map_lte = {0};

void add_forward_info(forward_info_map_t *map, int rnti, forward_info_t *info) {
    for (int i = 0; i < MAX_UE; i++) {
        if (map->rnti[i] == 0 || map->rnti[i] == rnti) {
            map->rnti[i] = rnti;
            map->info[i] = *info;

            LOG_E(PDCP, "Forward info added for rnti %u at index %d", rnti, i);
            LOG_E(PDCP, "  menb_ue_x2_id: %u", info->menb_ue_x2_id);
            LOG_E(PDCP, "  sgnb_ue_x2_id: %u", info->sgnb_ue_x2_id);
            LOG_E(PDCP, "  rnti: %lx", info->rnti);
            LOG_E(PDCP, "  assoc_id: %u\n", info->assoc_id);
            return;
        }
    }
    LOG_E(PDCP, "Failed to add forward info for rnti %u: map is full", rnti);
}

forward_info_t* get_forward_info(forward_info_map_t *map, int rnti) {
    for (int i = 0; i < MAX_UE; i++) {
        if (map->rnti[i] == rnti) {
            return &map->info[i];
        }
    }
    LOG_E(PDCP, "No matching forward info for %d\n", rnti);
    return NULL;
}

void remove_forward_info(forward_info_map_t *map, int rnti) {
    LOG_E(PDCP,"Start to remove ue %d", rnti);
    for (int i = 0; i < MAX_UE; i++) {
        if (map->rnti[i] == rnti) {
            forward_info_t *info = get_forward_info(&g_forward_info_map, rnti);
            if (info == NULL) {
              LOG_E(PDCP, "[NR] Cannot remove ue_id %d\n", rnti);
              return;
            }
            /* QCON Stage 8: drop the DC_user from the RIC publisher's
             * users[] array BEFORE freeing it via remove_dc_user(), so
             * the publisher thread does not iterate a freed pointer or
             * publish stale entries. Without this, every Pixel re-attach
             * leaks a stale DC_user — the publisher then emits multiple
             * kpm payloads per UE with mismatched target_split_ratio,
             * and split_ratio commands match the first stale instance. */
            unregister_dc_user_from_ric(info->user);
            remove_dc_user(info->user);
            map->rnti[i] = 0;
            memset(&map->info[i], 0, sizeof(forward_info_t));
            return;
        }
    }
}

// PAVE: LTE also need PDCP header!
...
void nr_pdcp_dc_info_update (int ue_id, int acked_bytes, int queue_size)
{
  forward_info_t *info = get_forward_info(&g_forward_info_map, ue_id);

  if (info == NULL) {
    LOG_E(PDCP, "[NR] Forward info is NULL for ue_id %d\n", ue_id);
    return;
  }
  update_nr_info(info->user, acked_bytes, queue_size);
}

void lte_pdcp_dc_info_update (int ue_id, int acked_bytes, int queue_size)
{
  forward_info_t *info = get_forward_info(&g_forward_info_map, ue_id);

  if (info == NULL) {
    LOG_E(PDCP, "[LTE] Forward info is NULL for ue_id %d\n", ue_id);
    return;
  }
  update_lte_info(info->user, acked_bytes, queue_size);
}
/* ===========================================================================
 * QCON Stage 6b — MCS+RB based bandwidth estimation
 *
 * Iterates the gNB MAC UE list, reads per-UE sched_pdsch (latest scheduling
 * decision: MCS, rbSize, Qm, R, layers, dl_bler) and ships a JSON KPI to RIC
 * via deliver_mcs_rb_to_ric().
 *
 * Called from nr_pdcp_ran_func.cpp's KPM publisher thread (~100ms cadence)
 * — does NOT touch the per-slot scheduler hot path.
 * ========================================================================= */
void qcon_emit_mac_kpm(void)
{
  ...
  UE_iterator(mac->UE_info.list, UE) {
    ...
    NR_sched_pdsch_t *p = &sched_ctrl->sched_pdsch;
    NR_UE_DL_BWP_t *bwp = &UE->current_DL_BWP;
    /* Publish even when rbSize=0 → RIC sees UE presence + last MCS. */

    /* bits per resource element = Qm * (R/1024) where R is OAI-encoded code rate */
    double bits_per_re = (p->Qm > 0 && p->R > 0) ? ((double)p->Qm * (double)p->R / 1024.0) : 0.0;
    /* slots/sec = 1000 * 2^numerology — n78 30kHz SCS → 2000 */
    int scs_idx = bwp ? bwp->scs : 1;
    int slots_per_sec = 1000 * (1 << scs_idx);
    int bler_pct = (int)(sched_ctrl->dl_bler_stats.bler * 100.0);

    /* leg = 1 (NR). LTE leg published by lte-softmodem in a future commit. */
    deliver_mcs_rb_to_ric((int)UE->rnti, /* leg */ 1,
                          p->mcs, p->rbSize, p->nrOfLayers ? p->nrOfLayers : 1,
                          bits_per_re, slots_per_sec, bler_pct);
  }
