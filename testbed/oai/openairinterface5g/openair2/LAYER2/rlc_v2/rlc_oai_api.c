  ...
  crc_t                    *crcs_pP)
{
  ...
  if (rb != NULL) {
    ...
    /* Deliver information for dual-connectivity multi-link routing.*/
    /* Current version, dual-connectivity multi-link routing deployed only for monolithic gNB now, */
    rlc_entity_buffer_status_t buf_stat = rb->buffer_status(rb, 1000*1000);
    int tx_bytes = buf_stat.total_tx_bytes - rb->last_total_tx_bytes;
    int txbuf_occ_bytes = buf_stat.tx_size;// + buf_stat.retx_size;

    MessageDef *msg;
    
    msg = itti_alloc_new_message(TASK_RLC_ENB, 0, X2AP_ENDC_LTE_INFO_INDICATION);
    X2AP_ENDC_LTE_INFO_INDICATION(msg).rnti          = ue->rnti;
    X2AP_ENDC_LTE_INFO_INDICATION(msg).rb_id        = 0; // not used
    X2AP_ENDC_LTE_INFO_INDICATION(msg).sdu_size = tx_bytes;
    X2AP_ENDC_LTE_INFO_INDICATION(msg).total_buf = txbuf_occ_bytes;

    itti_send_msg_to_task(TASK_RRC_ENB, 0, msg);

    rb->last_total_tx_bytes = buf_stat.total_tx_bytes;
  } else {
                             ...
                             const uint32_t *const destinationL2Id)
{
  ...
  if (rb != NULL) {
    ...
    LOG_E(RLC, "%s:%d:%s: fatal: SDU sent to unknown RB %d\n", __FILE__, __LINE__, __FUNCTION__, rb_idP);
    exit(1);
  }
