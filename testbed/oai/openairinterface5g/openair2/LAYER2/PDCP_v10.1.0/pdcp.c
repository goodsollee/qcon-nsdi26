...
//-----------------------------------------------------------------------------
{
  ...
  // PAVE: Use transort mode for split bearer in LTE
  if (modeP == PDCP_TRANSMISSION_MODE_TRANSPARENT) {
    ...
    if (pdcp_pdu_p != NULL) {
      ...
      //LOG_E(PDCP, "RNTI is %x", ctxt_pP->rntiMaybeUEid);
      // PAVE:MBMS_FLAG_YES in original. Modified for test
      rlc_status =
          rlc_data_req(ctxt_pP, srb_flagP, MBMS_FLAG_NO, rb_idP, muiP, confirmP, sdu_buffer_sizeP, pdcp_pdu_p, NULL, NULL);
    } else {
    ...
    switch (pdcp_p->rlc_mode) {
      case RLC_MODE_AM: {
        ...
        if (sequence_number - pdcp_p->last_submitted_pdcp_rx_sn > reordering_window ||
          ...
          LOG_E(PDCP,
            PROTOCOL_PDCP_CTXT_FMT"discard PDU, sequence number out of range: current SN = %d, last submitted SN = %d\n",
            PROTOCOL_PDCP_CTXT_ARGS(ctxt_pP, pdcp_p), sequence_number, pdcp_p->last_submitted_pdcp_rx_sn);
          LOG_E(PDCP, "Ignoring PDU due to SN mismatch: current SN = %d, reordering window = %d\n",
                sequence_number, reordering_window);
          free(sdu_buffer_pP);
          /* TODO: indicate integrity verification failure to upper layer */
          return false;
    ...
    if (msg_p != NULL) {
      switch (ITTI_MSG_ID(msg_p)) {
        case RRC_DCCH_DATA_REQ:
          ...
          LOG_E(PDCP, "Before calling pdcp_data_req from pdcp_run! RRC_DCCH_DATA_REQ (msg_p).rb_id: %ld \n", RRC_DCCH_DATA_REQ (msg_p).rb_id);
        ...
        case RRC_PCCH_DATA_REQ: {
          ...
          //paging pdcp log
          LOG_E(PDCP, "PDCP Received RRC_PCCH_DATA_REQ CC_id %d length %d \n", CC_id, sdu_buffer_sizeP);
        }
        break;
