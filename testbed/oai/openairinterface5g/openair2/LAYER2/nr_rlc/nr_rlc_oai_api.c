                      ...
                      crc_t *crcs_pP)
{
  ...
  if (rb != NULL) {
    ...
    // TODO: 240929 --> Need to move this function to operate when RLC ACK processed
    ...
    if (!NODE_IS_DU(type)) {
      ...
      //nr_rlc_statistics_t out = {0};
      //rb->get_stats(rb, &out);

      nr_rlc_entity_buffer_status_t buf_stat = rb->buffer_status(rb, 1000*1000);
      int total_tx_bytes = buf_stat.total_tx_bytes;
      int txbuf_occ_bytes = buf_stat.tx_size + buf_stat.retx_size;

      //nr_pdcp_dc_info_update(ue_id, out.txsdu_bytes - prev_txsdu_bytes, out.txbuf_occ_bytes);
      nr_pdcp_dc_info_update(ue_id, total_tx_bytes - rb->last_total_tx_bytes, txbuf_occ_bytes);

      rb->last_total_tx_bytes = total_tx_bytes;
    }
