...
void mac_top_init_eNB(void)
{
  ...
  /* QCON Stage 6b LTE-side: bring up the eNB→RIC ZMQ DEALER + 100ms
   * publisher thread. Mirrors the gNB-side init_ran_function() but
   * publishes mcs_rb with leg=0 (LTE leg). Idempotent — safe to leave
   * even if RIC isn't reachable (zmq_connect succeeds even without a
   * peer; messages just queue locally). */
  extern void lte_init_ran_function(void);
  lte_init_ran_function();
}
