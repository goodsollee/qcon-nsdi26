...
//------------------------------------------------------------------------------
{
  ...
  // PAVE: Resource preallocation disabled for DRX operation
  if (UE_template->scheduled_ul_bytes < UE_template->estimated_ul_buffer ||
      ...
      (UE_sched_ctl->cqi_req_timer > 500 && rrc_status >= RRC_CONNECTED)) { // cqi req timer expired long ago (do not put too low value, PAVE: 500, origin: 300)
