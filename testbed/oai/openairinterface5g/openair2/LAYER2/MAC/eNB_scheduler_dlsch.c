    ...
    if (round_DL != 8) {
      ...
      if (ta_len + sdu_length_total + header_length_total > 0) {
        ...
        if (ue_sched_ctrl->cdrx_configured) {
          ...
          //LOG_E(MAC, "DRX reset from ta %d, sdu %d, header %d\n", ta_len,sdu_length_total,header_length_total);
          /*
           * Note: contrary to the spec drx_retransmission_timer[harq_pid] is reset not stop.
           */
    ...
    if (subframeP == 5) { // MPDCCH
      ...
      if (round_DL == 8) {
        ...
        if ((sdu_length_total + header_len_dcch + header_len_dtch) > 0) {
          ...
          LOG_E(MAC, "[eNB %d] CC_id %d Generated DLSCH header (mcs %d, TBS %d, nb_rb %d)\n",
                module_idP,
                CC_id,
                mcs,
                TBS,
