...
int sctp_itti_send_new_message_forward(task_id_t task_id,
                                   instance_t instance,
                                   sctp_assoc_t assoc_id,
                                   uint8_t *buffer,
                                   uint32_t buffer_length,
...
{
  MessageDef      *message_p;
  sctp_data_forward_t *sctp_data_forward_p;

  message_p = itti_alloc_new_message(TASK_SCTP, 0, SCTP_DATA_FORWARD);

  sctp_data_forward_p = &message_p->ittiMsg.sctp_data_ind;

  sctp_data_forward_p->buffer = itti_malloc(TASK_SCTP, task_id, sizeof(uint8_t) * buffer_length);

  /* Copy the buffer */
  memcpy((void *)sctp_data_forward_p->buffer, (void *)buffer, buffer_length);

  sctp_data_forward_p->stream        = stream;
  sctp_data_forward_p->buffer_length = buffer_length;
  sctp_data_forward_p->assoc_id      = assoc_id;
