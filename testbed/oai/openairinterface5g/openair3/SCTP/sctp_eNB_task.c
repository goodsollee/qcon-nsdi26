...
// PAVE
static void sctp_forward_data (sctp_data_forward_t *sctp_data_forward_p)
{
    struct sctp_cnx_list_elm_s *sctp_cnx = NULL;

    DevAssert(sctp_data_forward_p != NULL);
    DevAssert(sctp_data_forward_p->buffer != NULL);
    DevAssert(sctp_data_forward_p->buffer_length > 0);

    sctp_cnx = sctp_get_cnx(sctp_data_forward_p->assoc_id, 0);

    if (sctp_cnx == NULL) {
        SCTP_ERROR("Failed to find SCTP description for assoc_id %d\n",
                   sctp_data_forward_p->assoc_id);
        /* TODO: notify upper layer */
        return;
    }

    if (sctp_data_forward_p->stream >= sctp_cnx->out_streams) {
        SCTP_ERROR("Requested stream (%"PRIu16") >= nb out streams (%"PRIu16")\n",
                   sctp_data_forward_p->stream, sctp_cnx->out_streams);
        return;
    }
     ...
     */
    if (sctp_sendmsg(sctp_cnx->sd, sctp_data_forward_p->buffer,
                     sctp_data_forward_p->buffer_length, NULL, 0,
                     htonl(sctp_cnx->ppid), 0, sctp_data_forward_p->stream, 0, 0) < 0) {
        SCTP_ERROR("Sctp_sendmsg failed: %s\n", strerror(errno));
        /* TODO: notify upper layer */
        return;
    }
    free(sctp_data_forward_p->buffer); // assuming it has been malloced
    LOG_E(SCTP, "[PAVE] Successfully sent %u bytes on stream %d for assoc_id %d\n",
               sctp_data_forward_p->buffer_length, sctp_data_forward_p->stream,
               sctp_cnx->assoc_id);
}
...
static int sctp_create_new_listener(const instance_t instance, const task_id_t requestor, sctp_init_t *init_p, int server_type)
{
  ...
  for (p = serv; p != NULL; p = p->ai_next) {
    ...
    int sendbuf = 262144; // 256 KB
    int recvbuf = 262144; // 256 KB
    setsockopt(sd, SOL_SOCKET, SO_SNDBUF, &sendbuf, sizeof(sendbuf));
    setsockopt(sd, SOL_SOCKET, SO_RCVBUF, &recvbuf, sizeof(recvbuf));

    int nodelay = 1;
    setsockopt(sd, IPPROTO_SCTP, SCTP_NODELAY, &nodelay, sizeof(nodelay));
    ...
    struct sctp_cnx_list_elm_s *sctp_cnx)
{
    ...
    if (flags & MSG_NOTIFICATION) {
        ...
        // General msgs
        // Data forwarding msgs from X2AP
        if (ntohl(sinfo.sinfo_ppid) == 27 && sinfo.sinfo_stream == 1) {       
            LOG_E(SCTP, "PPID (forward): %d, stream: %d", ntohl(sinfo.sinfo_ppid), sinfo.sinfo_stream);
            sctp_itti_send_new_message_forward(sctp_cnx->task_id,
                                       sctp_cnx->instance,
    ...
    if (received_msg != NULL) {
      LOG_D(SCTP,"Received message %d:%s\n",
        ...
        case SCTP_DATA_FORWARD: {
          sctp_forward_data (&received_msg->ittiMsg.sctp_data_forward);
        }
        break;
        default:
