...
int gtpv1u_create_s1u_tunnel(instance_t instance,
                             ...
                             gtpCallback callBack)
{
  LOG_E(GTPU, "[%ld] Start create tunnels for UE ID %u, num_tunnels %d, sgw_S1u_teid %x\n",
        instance,
  ...
  for (int i = 0; i < create_tunnel_req->num_tunnels; i++) {
    ...
    // Print sgw_addr and enb_addr with LOG_E
    char sgw_addr_str[INET_ADDRSTRLEN];
    char enb_addr_str[INET_ADDRSTRLEN];

    inet_ntop(AF_INET, create_tunnel_req->sgw_addr[i].buffer, sgw_addr_str, sizeof(sgw_addr_str));
    inet_ntop(AF_INET, addr, enb_addr_str, sizeof(enb_addr_str));

    LOG_E(GTPU, "Tunnel %d created: target Addr: %s, host Addr: %s\n", 
          i, sgw_addr_str, enb_addr_str);
  }
...
int gtpv1u_update_s1u_tunnel(
  ...
  LOG_E(GTPU, "[%ld] Start update tunnels for old RNTI %x, new RNTI %x, num_tunnels %d, sgw_S1u_teid %x, eps_bearer_id %x\n",
        instance,
        prior_rnti,
        create_tunnel_req->rnti,
        create_tunnel_req->num_tunnels,
...
int gtpv1u_create_ngu_tunnel(const instance_t instance,
                             ...
                             gtpCallbackSDAP callBackSDAP)
{
  LOG_E(GTPU, "[%ld] Start create tunnels for ue id %lu, num_tunnels %d, sgw_S1u_teid %x\n",
        instance,
...
int gtpv1u_update_ue_id(const instance_t instanceP, ue_id_t old_ue_id, ue_id_t new_ue_id)
{
  ...
  LOG_E(GTPU, "[%ld] Updated tunnels from UEid %lx to UEid %lx\n", instanceP, old_ue_id, new_ue_id);
  return !GTPNOK;
}

// PAVE additional implementation (Start from here 24/9/26)
// We first implement X2U tunnel for LTE --> NR to deliver control information (for multi-path routing)..
int gtpv1u_create_x2u_tunnel(
  const instance_t instance,
  const gtpv1u_enb_create_x2u_tunnel_req_t   *const create_tunnel_req,
  gtpv1u_enb_create_x2u_tunnel_resp_t *const create_tunnel_resp) {
  ...
  for (int i = 0; i < create_tunnel_req->num_tunnels; i++) {
    ...
    teid_t teid = newGtpuCreateTunnel(compatInst(instance),
                                      ...
                                      incoming_rb_id,
                                      create_tunnel_req->eps_bearer_id[i],
                                      create_tunnel_req->tenb_X2u_teid[i],
                                      -1, // no pdu session in 4G
    ...
    // Print sgw_addr and enb_addr with LOG_E
    char enb_addr_str[INET_ADDRSTRLEN];
    char gnb_addr_str[INET_ADDRSTRLEN];

    inet_ntop(AF_INET, create_tunnel_req->enb_addr[i].buffer, enb_addr_str, sizeof(enb_addr_str));
    inet_ntop(AF_INET, addr, gnb_addr_str, sizeof(gnb_addr_str));

    LOG_E(GTPU, "Tunnel %d created: target Addr: %s, host Addr: %s\n", 
          i, enb_addr_str, gnb_addr_str);
  }
...
int newGtpuDeleteOneTunnel(instance_t instance, ue_id_t ue_id, int rb_id)
{
  ...
  pthread_mutex_unlock(&globGtp.gtp_lock);
  LOG_E(GTPU, "Deleted tunnel TEID %d (RB %d) for ue id %ld, remaining bearers:\n", teid, rb_id, ue_id);
...
int gtpv1u_delete_s1u_tunnel( const instance_t instance,
  ...
  LOG_E(GTPU, "[%ld] Start delete tunnels for RNTI %x\n", instance, req_pP->rnti);
  pthread_mutex_lock(&globGtp.gtp_lock);
