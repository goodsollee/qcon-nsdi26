...
bool PhyReceiverModule::processReceivedPacket(const unsigned char* packet, size_t len) {
    ...
    if (!is_ip_encapsulated) {
        LOG_DEBUG("[PHY Receiver] Packet is not IP-encapsulated, processing as is");
        ...
        if (len <= ip_header_len) {
            LOG_WARN("[PHY Receiver] Invalid IP header length");
            return false;
        }
        
        // Skip outer IP header, only use inner packet (PDCP + inner IP)
        processed_packet = packet + ip_header_len;
        processed_len = len - ip_header_len;
        
        LOG_DEBUG("[PHY Receiver] Decapsulated outer IP header, remaining length: " << processed_len);
    }
