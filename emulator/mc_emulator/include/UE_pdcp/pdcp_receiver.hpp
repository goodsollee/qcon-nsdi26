...
private:
    mutable std::mutex callbackMutex_;  // new
    ...
    // Sequence counters
    std::atomic<uint64_t> nextExpected;    // Next sequence number expected
    std::atomic<uint64_t> highestReceived; // Highest sequence received
    std::atomic<uint64_t> highestEpoch;    // Highest epoch received
    ...
    // Statistics
    std::atomic<uint64_t> deliveredCount;
    std::atomic<uint64_t> droppedCount;
