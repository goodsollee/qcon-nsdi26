 ...
 */
static void dump_ip_packet(const unsigned char *buf, size_t len, const char *direction) {
    if (len < 20) {
        fprintf(stderr, "[%s] Packet too short: %zu bytes\n", direction, len);
        return;
    }
    
    const struct ipv4_hdr *iph = (const struct ipv4_hdr *) buf;
    
    if (iph->version != 4) {
        fprintf(stderr, "[%s] Not IPv4 packet (version=%d)\n", direction, iph->version);
        return;
    }
    
    int ihl_bytes = iph->ihl * 4;
    if (ihl_bytes < 20 || ihl_bytes > (int)len) {
        fprintf(stderr, "[%s] Bad IHL=%d, pkt_len=%zu\n", direction, ihl_bytes, len);
        return;
    }
    ...
    fprintf(stderr, "[%s] IPv4 src=%s -> dst=%s, proto=%u(%s), ttl=%u, tot_len=%u\n",
            direction,
            src_ip, dst_ip,
            iph->protocol, proto_name,
            iph->ttl,
            ntohs(iph->tot_len));
}
...
static ssize_t process_packet_userspace(unsigned char *packet, size_t len, const char *direction, bool is_outgoing) {
    ...
    if (!is_outgoing) {
        // For incoming traffic, we just forward as-is (bypass userspace processing)
        return len;
    }
    ...
    dump_ip_packet(packet, len, direction);
    
    struct ipv4_hdr *iph = (struct ipv4_hdr *)packet;
    
    // Only process IPv4 packets
    if (iph->version != 4) {
        return len; // Pass through non-IPv4 packets unchanged
    }
    
    // Example: simple TTL adjustment
    iph->ttl = 64; // Set consistent TTL
    
    // Recalculate checksum after modifications
    iph->check = 0;
    iph->check = ip_checksum(iph, iph->ihl * 4);
    
    // Return modified packet length (or 0 to drop)
    return len;
}
